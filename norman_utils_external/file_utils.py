...
class FileUtils(metaclass=Singleton):
    ...
    def get_file_type_from_header(self, header: bytes):
        ...
        if hex_header.startswith("fff1") or hex_header.startswith("fff9"):  # aac
            container_modality, container_encoding, mime_type, file_extension = "Audio", "aac", "audio/aac", "aac"
        elif hex_header.startswith("0b77"):  # ac3 sync word
            container_modality, container_encoding, mime_type, file_extension = "Audio", "ac3", "audio/ac3", "ac3"
        elif hex_header.startswith("664c6143"):  # flac - "fLaC"
            container_modality, container_encoding, mime_type, file_extension = "Audio", "flac", "audio/flac", "flac"
        elif hex_header.startswith("494433"):  # mp3 - ID3 tag
            container_modality, container_encoding, mime_type, file_extension = "Audio", "mp3", "audio/mpeg", "mp3"
        elif hex_header.startswith("4f676753") and "4f70757348656164" in hex_header:  # opus - OggS + OpusHead
            container_modality, container_encoding, mime_type, file_extension = "Audio", "opus", "audio/opus", "opus"
        elif hex_header.startswith("4f676753") and "01766f72626973" in hex_header:  # vorbis - OggS + \x01vorbis
            container_modality, container_encoding, mime_type, file_extension = "Audio", "vorbis", "audio/vorbis", "ogg"
        elif hex_header.startswith("52494646") and "57415645" in hex_header:  # wav - RIFF + WAVE
            container_modality, container_encoding, mime_type, file_extension = "Audio", "wav", "audio/wav", "wav"

        # Image (alphabetical: jpg, png, webp)
        elif hex_header.startswith("ffd8ff"):  # jpg
            container_modality, container_encoding, mime_type, file_extension = "Image", "jpg", "image/jpeg", "jpg"
        elif hex_header.startswith("89504e47"):  # png
            container_modality, container_encoding, mime_type, file_extension = "Image", "png", "image/png", "png"
        elif hex_header.startswith("52494646") and "57454250" in hex_header:  # webp - RIFF + WEBP
            container_modality, container_encoding, mime_type, file_extension = "Image", "webp", "image/webp", "webp"
        ...
        # Video (alphabetical: avi, matroska, mov, mp4, ogg, webm)
            ...
            container_modality, container_encoding, mime_type, file_extension = "Video", "avi", "video/x-msvideo", "avi"
        elif hex_header.startswith("1a45dfa3") and "7765626d" not in hex_header:  # matroska - EBML without webm doctype
            container_modality, container_encoding, mime_type, file_extension = "Video", "matroska", "video/x-matroska", "mkv"
        elif hex_header.startswith("000000") and "6674797071742020" in hex_header:  # mov - ftyp qt (QuickTime)
            container_modality, container_encoding, mime_type, file_extension = "Video", "mov", "video/quicktime", "mov"
        elif hex_header.startswith("000000") and "66747970" in hex_header:  # mp4 - ftyp (mp4 and variants)
            container_modality, container_encoding, mime_type, file_extension = "Video", "mp4", "video/mp4", "mp4"
        elif hex_header.startswith("4f676753"):  # ogg - OggS (generic, no vorbis/opus detected)
            container_modality, container_encoding, mime_type, file_extension = "Video", "ogg", "video/ogg", "ogg"
        elif hex_header.startswith("1a45dfa3") and "7765626d" in hex_header:  # webm - EBML + webm doctype
            container_modality, container_encoding, mime_type, file_extension = "Video", "webm", "video/webm", "webm"

        # File (zip-based formats: jit, pt, zip - all return bin)
        elif hex_header.startswith("504b0304"):  # `.pt` files have a zip header
            container_modality, container_encoding, mime_type, file_extension = "File", "zip", "application/octet-stream", "zip"

        # Text (must be last - uses fallback decode detection)
        elif self.__is_utf16(header):
            container_modality, container_encoding, mime_type, file_extension = "Text", "utf16", "text/plain", "txt"
        elif self.__is_utf8(header):
            container_modality, container_encoding, mime_type, file_extension = "Text", "utf8", "text/plain", "txt"
        else:
            container_modality, container_encoding, mime_type, file_extension = "File", "bin", "application/octet-stream", "bin"
